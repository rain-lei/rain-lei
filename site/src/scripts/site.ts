...
const repoTarget=document.getElementById('githubRepos');
if(repoTarget){const fallback=[['rain-lei','个人网站、博客源码与 GitHub 资料页','JavaScript',6],['easyxgame','C++17 / EasyX 课程作业与原创游戏','C++',3],['EDAbackend-todo-practice','Express 后端练习：JWT 鉴权与 Todo CRUD','JavaScript',2]];const esc=(v:unknown)=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]!));const render=(repos:any[])=>{repoTarget.innerHTML=repos.slice(0,3).map((r,i)=>`<a class="github-repo-card" href="https://github.com/rain-lei/${esc(r.name)}" target="_blank" rel="noopener noreferrer"><span class="repo-index">0${i+1}</span><span class="repo-card-content"><span class="repo-title-row"><span class="repo-title-wrap"><strong>${esc(r.name)}</strong>${r.name==='rain-lei'?'<em class="repo-badge">本站</em>':''}</span><span class="repo-arrow">↗</span></span><span class="repo-description">${esc(r.description||'代码、实验与持续迭代的项目记录。')}</span><span class="repo-meta"><span>${esc(r.language||'多语言')}</span><span>★ ${Number(r.stargazers_count)||0}</span></span></span></a>`).join('');};fetch('https://api.github.com/users/rain-lei/repos?sort=pushed&direction=desc&per_page=20').then(r=>r.ok?r.json():Promise.reject()).then((repos:any[])=>render(repos.filter(r=>!r.fork&&!r.archived))).catch(()=>render(fallback.map(([name,description,language,stargazers_count])=>({name,description,language,stargazers_count}))));}
...
let active='all';const filterArchive=()=>{const q=(archiveSearch?.value||'').trim().toLowerCase();let count=0;archiveItems.forEach(item=>{const visible=(active==='all'||item.dataset.category===active)&&(!q||(item.textContent||'').toLowerCase().includes(q));item.hidden=!visible;if(visible)count++;});if(resultCount)resultCount.textContent=`${count} 篇结果`;};
filters.forEach(button=>button.addEventListener('click',()=>{active=button.dataset.filter||'all';filters.forEach(b=>b.classList.toggle('is-active',b===button));filterArchive();}));archiveSearch?.addEventListener('input',filterArchive);filterArchive();
